---
title: "AI - Deep Learning Conference"
author: "Trenton Pulsipher"
date: "5/24/2019"
output: 
  revealjs::revealjs_presentation:
    theme: simple
    highlight: kate
    reveal_options:
      slideNumber: true
      previewLinks: true
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
```

# Intro {data-background="HS1slidetemplate2.png"}

## What do you know of deep learning? {data-background="HS1slidetemplate.png"}

![](deep_learning_pit.png)

## What is AI? ML? DL? NN? {data-background="HS1slidetemplate.png"}

![](AI_algorithms.png)


## What is AI? ML? DL? NN? {data-background="HS1slidetemplate.png" fig.height="100%"}

![](machine_learning_algorithms.jpg)


## Define AI, ML, DL, NN {data-background="HS1slidetemplate.png"}


## Speakers {data-background="HS1slidetemplate.png"}

- Salesforce
- Twitter
- Change Healthcare
- Facebook
- Fidelity
- GE
- Research hospitals
- CVS / Aetna


## Topics {data-background="HS1slidetemplate.png"}

- AI Ethics
- Embedding
- Speech to Text (language agnostic)
- Object Recognition (eye tracking)
- Attention Mapping (Adobe, focus vs periphery)
- Model Governance
- Cautionary Tales


## Outline {data-background="HS1slidetemplate.png"}

1. Immediate Opportunities
2. Governance
3. Coming Opportunities & Cool Stuff


# Immediate Opportunities {data-background="HS1slidetemplate2.png"}

## Embedding {data-background="HS1slidetemplate.png"}

> Definition: 
> Convert Discrete to Continuous

Uses include:

- Building model features
- Feature compression
- Nearest neighbor search
- Transfer learning


## Speech to Text (Deep Speech) {data-background="HS1slidetemplate.png"}

- Mozilla Firefox
- 10,000 hours required to train the models adequately
- Iara Health: patient record transcribing
    + **Webinars?**
- Common Voice is their tool to crowdsource validation of models
- Future is to convert to Unicode to cover all languages into one model

## BOLA.AI {data-background="HS1slidetemplate.png"}

- Voice charting assistant (think periocharts)
- Boston based four person team
- Contracts alreay with Eaglesoft, Curve, and 2nd largest DSO.
- Could do native integration in EHR


# Governance {data-background="HS1slidetemplate2.png"}

## AI Ethics in Product Development {data-background="HS1slidetemplate.png"}

Must maintain ethics in

- Content / Product
- Process

Ethics are defined as:

- As beneficial as possible
- Respect human agency
- Allow human development
- Ensure social justice


## AI Ethics Concerns in Dentistry {data-background="HS1slidetemplate.png"}

- Who's most likely to pay?
- Marketing to potential patients?
- Zip code / Income / Ethnicity


## Model Governance {data-background="HS1slidetemplate.png"}

Quant University Data Flow recreated in Lucid
they developed their own process/tool, QuSandbox


## Model Governance Considerations {data-background="HS1slidetemplate.png"}

- Build models for all parts of the DS process
- Stress & QC test (watch for model decay)
- Can we file IP/Patent the product or algorithm
- Package management
- Hardware needs
- OECD principles standard on AI for Governance

## 4 V's of Big Data {data-background="HS1slidetemplate.png"}

![](4-Vs-of-big-data.jpg){}

# Coming Opportunities & Cool Stuff {data-background="HS1slidetemplate2.png"}

## AI for Creativity {data-background="HS1slidetemplate.png"}

Human visual system / object recognition shows that we see detail in the middle but not on the periphery.
**See if I can find the Adobe example Zoya ?**
Eye tracking is expensive and difficult. However, Adobe is building a real-time reflow design based on deep learning results. Meaning you change A/B testing to real-time design optimization, or attention mapping.

## Attention Mapping {data-background="HS1slidetemplate.png"}

Not just A/B testing - **any graphic design**

[Zoya](web.mit.edu/zoya/www/docs/UIST2017-importance-reducedfilesize.pdf)



## Challenges {data-background="HS1slidetemplate.png"}

- Calculating costs for being wrong
    + acting on false positives
    + not acting on false negatives


## Suggestions {data-background="HS1slidetemplate.png"}

- Keep Human-in-the-Loop
- Focus on "better" not cheaper
- Need to go live fast, then learn from it
